import { Client, registry, MissingWalletError } from 'anpan2val-loan-client-ts'

import { Loan } from "anpan2val-loan-client-ts/anpan2val.loan.loan/types"
import { Params } from "anpan2val-loan-client-ts/anpan2val.loan.loan/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: anpan2val.loan.loan initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.Anpan2ValLoanLoan.query.queryParams()).data
		...
		async QueryLoan({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.Anpan2ValLoanLoan.query.queryLoan( key.id)).data
		...
		async QueryLoanAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.Anpan2ValLoanLoan.query.queryLoanAll(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.Anpan2ValLoanLoan.query.queryLoanAll({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async sendMsgApproveLoan({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.Anpan2ValLoanLoan.tx.sendMsgApproveLoan({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async sendMsgRequestLoan({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.Anpan2ValLoanLoan.tx.sendMsgRequestLoan({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async MsgApproveLoan({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.Anpan2ValLoanLoan.tx.msgApproveLoan({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgApproveLoan:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgRequestLoan({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.Anpan2ValLoanLoan.tx.msgRequestLoan({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgRequestLoan:Init Could not initialize signing client. Wallet is required.')
